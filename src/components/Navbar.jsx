        ...
        <div className="flex flex-col h-full pt-24 px-6 pb-8">
          <div className="flex-1 space-y-2">
            {navLinks.map((link, index) => (
              <a key={link.id} href={`#${link.id}`} onClick={(e) => { e.preventDefault(); scrollToSection(link.id) }}
                className={`block py-4 px-4 rounded-lg text-lg transition-all duration-200 transform ${isMobileMenuOpen ? 'translate-x-0 opacity-100' : 'translate-x-8 opacity-0'} ${activeSection === link.id ? 'bg-primary-500/20 text-primary-400 border-l-2 border-primary-500' : 'text-gray-300 hover:bg-dark-700/50 hover:text-white hover:pl-6'}`}
          ...
          <div className="flex justify-center gap-6 mb-6">
            <a href="https://www.linkedin.com/in/prycehedrick/" target="_blank" rel="noopener noreferrer" 
               className="p-3 min-w-[44px] min-h-[44px] flex items-center justify-center text-gray-400 hover:text-primary-400 transition-colors rounded-lg hover:bg-dark-700/50" 
               aria-label="LinkedIn">
          ...
          <button onClick={() => scrollToSection('contact')} className={`btn-primary w-full py-4 text-base transform transition-all duration-300 ${isMobileMenuOpen ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'}`} style={{ transitionDelay: '350ms' }}>Book a Call</button>
